/*
 * 2014-03-10 changes for "stone for iOS" => preprocessor identifier "IOS_GUI"
 *            by TANABE Makoto (KLab Inc.)
 */
...
#ifdef IOS_GUI
char *OutMessageBufPtr = NULL;
int OutMessageBufLength = 0;
#include <setjmp.h>
jmp_buf envbuf;
char *stoneCvsId() {return CVS_ID;}
#define exit_stone(n) longjmp(envbuf, 1)
#else
#define exit_stone(n) exit(n)
#endif // IOS_GUI
...
#ifdef IOS_GUI
    if (OutMessageBufPtr && OutMessageBufLength > 0) {
        vsnprintf(OutMessageBufPtr, OutMessageBufLength, fmt, ap);
    }
#endif
    va_end(ap);
    str[LONGSTRMAX] = '\0';
...
XHosts *mkXhosts(int nhosts, char *hosts[], sa_family_t family, char *mesg) {
	...
	if (!strcmp(hosts[i], "!")) {
		...
		*p++ = '\0';
	    ...
	    sa->sa_family = family;
	    if (host2sa(xhost, NULL, sa, &salen, NULL, NULL, 0)) exit_stone(1);
...
#else
int main_stone(int argc, char *argv[], char *outbufptr, int outbuflen) {
#endif
#ifdef IOS_GUI
    if (outbufptr && outbuflen > 0) {
        OutMessageBufPtr = outbufptr;
        OutMessageBufLength = outbuflen;
    }
    if (setjmp(envbuf) != 0) {
        Debug = 0;
        return 1;
